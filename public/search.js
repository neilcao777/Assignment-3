...
async function find_list(pokemonTypeList) {
  ...
  for (i = 0; i < pokemonList.length; i++) {
    ...
    show_pokemon += `<div class='image_container' id='${pokemon_name}> <a href="/profile/${pokemon_id}"> ${pokemon_name} <img src="${pokemon_photo}"></a> </div>`;
...
function insert_profile_event_to_timeline() {
  // pokeName = $(`#${pokemon_name}`).attr("id");
  pokeName = this.id;
  ...
  jQuery.ajax({
    ...
    type: "put",
    data: {
      text: `The user has clicked on ${pokeName}'s profile.`,
      time: `${now}`,
      hits: 1,
    },
    success: function (r) {
